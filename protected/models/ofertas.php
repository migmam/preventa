...
class ofertas extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('oferta', 'required'),
			array('oferta', 'length', 'max'=>45),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, oferta', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('oferta',$this->oferta,true);

		return new CActiveDataProvider('ofertas', array(
			'criteria'=>$criteria,
		));
	}
